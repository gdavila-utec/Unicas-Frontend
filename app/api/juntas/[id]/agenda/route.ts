...
export async function GET(
  ...
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL}/agenda/junta/${params.id}`,
    {
      headers: {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/agenda/`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
    ...
    return NextResponse.json(
      { error: 'Failed to post agenda item' },
      { status: 500 }
    );
  }
}
...
export async function PUT(
  ...
  try {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/agenda/${id}/`,
      {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
